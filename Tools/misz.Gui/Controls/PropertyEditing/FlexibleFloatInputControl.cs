...
namespace misz.Gui
{
    ...
    public class FlexibleFloatInputControl : UserControl
    {
        ...
        public FlexibleFloatInputControl( float value, float min, float max, float softMin, float softMax, float stepSize = 0, bool checkBox = false )
        {
            if ( min >= max )
            ...
            m_min = min;
            m_max = max;
            m_softMin = MathUtil.Max<float>( softMin, min );
            m_softMax = MathUtil.Min<float>( softMax, max );
            ...
            {
                FlexibleFloatEditorForm dialog = new FlexibleFloatEditorForm( m_min, m_max, m_softMin, m_softMax, m_stepSize );
                dialog.Text = "Customize - " + PropertyName;
                dialog.StartPosition = FormStartPosition.CenterParent;
                dialog.ShowDialog();
                if ( dialog.DialogResult == DialogResult.OK )
                {
                    SetValue2( m_value, dialog.SoftMin, dialog.SoftMax, dialog.StepSize, false );
                }
            };
        ...
        private void SetValue2( float value, float softMin, float softMax, float stepSize, bool force )
        {
            value = MathUtil.Clamp<float>( value, softMin, softMax );
            value = MathUtil.Clamp<float>( value, m_min, m_max );
