		...
		IInstancingContext
	{
		...
		public bool CanInsert2( DragEventArgs e )
		{
			...
			if ( items != null
				&& items.Length == 1
				&& AreTimelineItems( items )
				&& ( TimelineControl.TargetGroup == null || TimelineControl.IsEditable( TimelineControl.TargetGroup ) )
				)
			{
				...
				object ob = items[0];
				DomNode obDomNode = ob as DomNode;
				...
				else
				{
					...
					if ( (groupCamera = obDomNode.As<GroupCamera>()) != null )
					{
						foreach ( DomNode child in m_timelineDocument.DomNode.Children )
						{
							GroupCamera otherGroupCamera = child.As<GroupCamera>();
				...
				////GroupCamera groupCamera = obDomNode.As<GroupCamera>();
				////if ( groupCamera != null )
				//TrackFader trackFader = null;
				//if ( (groupCamera = obDomNode.As<GroupCamera>()) != null )
				//{
				//	if ( dropTarget == null )
				//	{
				...
				//	}
				//}
				//else if ( (trackFader = obDomNode.As<TrackFader>()) != null )
				//{
				//	if ( dropTarget != null )
				//	{
				//		groupCamera = dropTarget.As<GroupCamera>();
				//		if ( groupCamera != null )
				//		{
				//			return true;
				//		}
